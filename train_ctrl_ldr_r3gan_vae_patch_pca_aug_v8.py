...
class Decoder(nn.Module):
    def __init__(self,latent_dim=128,hidden_dim=256):
        ...
        self.dense = nn.Linear(latent_dim, 4 * 4 * hidden_dim)
        self.reshape = Reshape((hidden_dim, 4, 4))
        
        # Three ResBlocks with upsampling (up 256)
        self.resblock1 = ResBlock(hidden_dim, hidden_dim, up=True)  # 4x4 -> 8x8
        self.resblock2 = ResBlock(hidden_dim, hidden_dim, up=True)  # 8x8 -> 16x16
        self.resblock3 = ResBlock(hidden_dim, hidden_dim, up=True)  # 16x16 -> 32x32
        
        # Final layers: BN, ReLU, 3x3 conv, Tanh
        self.bn = nn.BatchNorm2d(hidden_dim)
        self.relu = nn.ReLU()
        self.final_conv = nn.Conv2d(hidden_dim, 3, kernel_size=3, padding=1)
        self.tanh = nn.Tanh()
...
def train_ebm_gan(args):
    ...
    # Initialize models
    generator = Decoder(latent_dim=args.latent_dim,hidden_dim=args.hidden_dim).to(device)
        ...
        for i, (views, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast() if args.use_amp else nullcontext():
                    ...
                    z_aug = discriminator.net(aug_samples.detach()).squeeze()
                    ...
                    loss_cos1 = 1-F.cosine_similarity(z1,z2.detach(),dim=-1).mean()
                    loss_cos2 = 1-F.cosine_similarity(z2,z1.detach(),dim=-1).mean()
                    ...
                    z_noised = torch.randn_like(z_aug)
                    ...
                    d_loss_cos = 1 - F.cosine_similarity(z_fake,z.detach(),dim=-1).mean()
                    ...
                    d_loss = d_loss.mean() + loss_tcr #+ loss_dino #+ d_loss_cos
                if args.use_amp:
            ...
            with autocast() if args.use_amp else nullcontext():
                ...
                loss_cos = 1 - F.cosine_similarity(z_fake,z.detach(),dim=-1).mean()
