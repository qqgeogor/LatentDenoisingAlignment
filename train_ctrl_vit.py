    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, (real_samples, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):  # Train discriminator more frequently
                ...
                z_upsampled = generator_projector(z)
                z_upsampled = torch.cat([z.unsqueeze(1),z_upsampled],dim=1)
                fake_samples = generator.forward_decoder(z_upsampled).detach().requires_grad_(True)
                fake_samples = generator.unpatchify(fake_samples)

                # Compute energies
                real_energy = discriminator.forward_feature(real_samples)[:,0]
                real_energy = discriminator.discriminator_head(real_energy)
                fake_energy = discriminator.forward_feature(fake_samples)[:,0]
                fake_energy = discriminator.discriminator_head(fake_energy)
        ...
        save_gan_samples(generator, discriminator, epoch, args.output_dir, generator_projector,real_samples=real_samples)
        ...
        if epoch % args.save_freq == 0:
            torch.save({
                ...
                'generator_projector_state_dict': generator_projector.state_dict(),
...
def save_gan_samples(generator, discriminator, epoch, output_dir, generator_projector, n_samples=36,real_samples=None):
    generator.eval()
    discriminator.eval()
