...
from train_mae_cifar10_jepa_spconv import MaskedAutoencoderViT
...
class CifarClassifier(nn.Module):
    def __init__(self, backbone, num_classes=10,freeze_backbone=False):
        ...
        # self.backbone.transfer_sparse_to_dense_params()
        
    def forward(self, x):
        # Get features from the backbone
        if self.freeze_backbone:
            with torch.no_grad():
                features = self.backbone.encoder.forward_feature(x).mean(1)
        else:
            features = self.backbone.encoder.forward_feature(x).mean(1)
