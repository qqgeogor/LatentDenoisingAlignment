...
def train_ebm_gan(args):
    ...
    momentum_scheduler = cosine_scheduler(
        base_value=args.ema_momentum, final_value=1., 
        epochs=args.epochs, niter_per_ep=len(trainloader))


    wd_scheduler = cosine_scheduler(
        base_value=args.weight_decay, final_value=args.final_weight_decay, 
        epochs=args.epochs, niter_per_ep=len(trainloader), warmup_epochs=0, start_warmup_value=args.weight_decay)
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, (views, _) in enumerate(tqdm(trainloader)):
            real_samples,aug_samples = views[:2]
            local_views = views[2:]

            batch_size = real_samples.size(0)
            real_samples = real_samples.to(device)
            aug_samples = aug_samples.to(device)
            local_samples = [v.to(device) for v in local_views]

            it = i + epoch * len(trainloader)
            momentum = momentum_scheduler[it]
            weight_decay = wd_scheduler[it]
            d_optimizer.param_groups[0]['weight_decay'] = weight_decay
            ...
            with autocast() if args.use_amp else nullcontext():
                ...
                Z_teachers = []
                for x in [real_samples,aug_samples]:
                    z = teacher_discriminator.forward_feature(x)[:,0]

                    Z_teachers.append(z)  

                Z_students = []
                for x in [real_samples,aug_samples]+local_samples:
                    z = discriminator.forward_feature(x)[:,0]
                    Z_students.append(z)
                ...
                for z in Z_students:
                    loss_tcr += -R(z)
                loss_tcr *=1e-2
                loss_tcr /=len(Z_students)

                loss_cos = 0
                for i,zt in enumerate(Z_teachers):
                    for j,zs in enumerate(Z_students):
                        if i==j:
                            continue
                        
                        
                        loss_cos += 1-F.cosine_similarity(zs,zt,dim=-1).mean()
                        
                loss_cos /=len(Z_students)

                d_loss = loss_tcr+loss_cos
