...
from vit import MaskedAutoencoderViT,PatchVariationalAutoencoder
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (batch, masks_enc, masks_pred) in enumerate(trainloader):
            ...
            with autocast() if args.use_amp else contextlib.nullcontext():
                ...
                z = model.forward_feature(imgs,masks=masks_enc)[:,1:,:]
                z = model.forward_predictor(z,masks_x=masks_enc,masks=masks_pred)
                
                
                with torch.no_grad():
                    # h = teacher_model.forward_feature(imgs,masks=masks_pred)[:,1:,:]
                    # h = F.layer_norm(h, (h.size(-1),))
                    # h = h.detach()

                    images_patch = model.patchify(imgs)
                    b,n,c = images_patch.shape
                    masked_patches = apply_masks(images_patch,masks_pred)
                    h = vae.encoder(masked_patches)
                    
                    h = F.layer_norm(h, (h.size(-1),))
                    h = h.detach()
                
                _, loss_tcr, loss_cos = simsiam_loss(z, h,lambda1=2e-2)
