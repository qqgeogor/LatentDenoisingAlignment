...
from train_ctrl_ldr_r3gan_vae import Encoder
from torch.cuda.amp import autocast, GradScaler
import mlflow
import mlflow.pytorch

class CifarClassifier(nn.Module):
    def __init__(self, backbone, num_classes=10,freeze_backbone=False,latent_dim=128):
        super().__init__()
...
def finetune(args):
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        backbone = Encoder(img_channels=3,latent_dim=args.latent_dim).to(device)
        ...
        # Create classifier
        model = CifarClassifier(backbone,freeze_backbone=args.freeze_backbone,latent_dim=args.latent_dim).to(device)
