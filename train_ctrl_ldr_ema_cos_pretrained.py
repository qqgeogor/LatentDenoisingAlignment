        ...
        for i, (views, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast() if args.use_amp else nullcontext():
                    ...
                    # real_energy_global = discriminator.head(z)
                    # fake_energy_global = discriminator.head(z_fake)
                    
                    # realistic_logits_global = real_energy_global - fake_energy_global
                    # d_loss_global = F.softplus(-realistic_logits_global).squeeze()



                    d_loss = d_loss #+ d_loss_global
                    
                    loss_tcr = -R(z).mean() #+ -R(z_fake).mean()
                    loss_tcr /=200
            ...
            with autocast() if args.use_amp else nullcontext():
                ...
                # g_loss += F.softplus(-realistic_logits_global).squeeze()
