...
from utils_ibot import cosine_scheduler
...
from karas_sampler import KarrasSampler,get_sigmas_karras,EDMPrecondMae
...
#         F_x = self.inner_model(x, noised_image, mask, ids_restore)
...
class DenoisingBlock(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_chans=3,embed_dim=192, decoder_embed_dim=96,decoder_depth=4,decoder_num_heads=3,mlp_ratio=4,norm_layer=nn.LayerNorm,use_checkpoint=False):
        ...
        # self.blocks = nn.ModuleList([ EDMPrecondNoProp(Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer)) for _ in range(decoder_depth)])
...
class MaskedDenoisingVit():
    ...
    def stochastic_iterative_sampler(
        ...
        # ts = torch.linspace(0,steps,steps+1)
        # t_max_rho = t_max ** (1 / rho)
        # t_min_rho = t_min ** (1 / rho)
        
        # for i in range(len(ts) - 1):
        #     t = (t_max_rho + ts[i] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        #     print(f'i: {i:.4f},t: {t:.4f},t_max_rho: {t_max_rho:.4f},t_min_rho: {t_min_rho:.4f}')

        #     x0 = model.denoise(x, latent, mask, ids_restore,t)
        #     next_t = (t_max_rho + ts[i + 1] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        #     next_t = np.clip(next_t, t_min, t_max)
        #     delta_t = (next_t**2 - t**2)**0.5   
        #     print(f'next_t: {next_t:.4f},t: {t:.4f},t_min: {t_min:.4f},t_max: {t_max:.4f},delta_t: {delta_t:.4f}')
        ...
        for i in range(self.T):
            ...
            if i < self.T - 1:
                ...
                x = x0 + torch.randn_like(x) * delta_t
                print(f'i: {i},next_t: {next_t:.4f},t: {t:.4f},t_min: {t_min:.4f},t_max: {t_max:.4f},delta_t: {delta_t:.4f}')
