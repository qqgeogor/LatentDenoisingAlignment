...
class Generator(nn.Module):
    ...
    def forward(self, z,use_vae=False):
        if use_vae:
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, up=False):
        ...
        # Shortcut connection
        self.shortcut = nn.Sequential()
        if stride != 1 or in_channels != out_channels or up:
            shortcut_layers = []
            if up:
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, (real_samples, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):  # Train discriminator more frequently
                ...
                fake_samples,loss_kld = generator(z,use_vae=False)
                ...
                # loss_cos1 = 1 - F.cosine_similarity(z_real.detach(),z_fake,dim=-1).mean()
                # loss_cos2 = 1 - F.cosine_similarity(z_real,z_fake.detach(),dim=-1).mean()
                
                # loss_cos = loss_cos1 + loss_cos2
                # loss_cos /=2
                ...
                # d_loss = d_loss.mean() + loss_tcr + loss_cos
            ...
            fake_samples,_ = generator(z,use_vae=False)
