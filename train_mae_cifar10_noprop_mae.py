...
class MaskedDenoisingVit():
    ...
    def stochastic_iterative_sampler(
        ...
        for i in range(self.T):
            ...
            if i < self.T - 1:
                ...
                x = x0 + torch.randn_like(x) * (next_t**2 - t**2)**0.5
            else:
                x = x0
            
            x = (1-mask.unsqueeze(-1)) * img_patch + mask.unsqueeze(-1) * model.patchify(x0)
            x = model.unpatchify(x)
            
            all_steps.append(x)

        out = (1-mask.unsqueeze(-1)) * img_patch + mask.unsqueeze(-1) * model.patchify(x)
    ...
    def sample_heun(
        ...
        for i in range(self.T):
            sigma = sigmas[i]
            sigma_next = sigmas[i + 1] if i < self.T - 1 else 0
