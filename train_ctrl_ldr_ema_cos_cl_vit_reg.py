        ...
        for i, (views, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast() if args.use_amp else nullcontext():
                    ...
                    z = discriminator.forward_feature(real_samples)

                    z_register = z[:,:1+generator.num_register_tokens]
                    
                    z = z[:,0]

                    # Add mask tokens
                    mask_tokens = generator.mask_token.repeat(z.shape[0],n, 1)
                    z_noised = torch.cat([z_register, mask_tokens], dim=1)
