...
class MaskedAutoencoderViT(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_chans=3,
        ...
        self.predictor_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)

        self.predictor_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, decoder_embed_dim), requires_grad=False)

        self.predictor_blocks = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
            for i in range(decoder_depth)])

        self.predictor_norm = norm_layer(decoder_embed_dim)
        
        self.predictor_pred = nn.Linear(decoder_embed_dim, decoder_embed_dim, bias=True)
    ...
    def forward_predictor(self, x,mask,ids_restore):
        ...
        for i, (imgs, _) in enumerate(trainloader):
            ...
            if args.use_amp:
                with autocast():
                    ...
                    loss_ijepa = F.smooth_l1_loss(z_pred, z_anchor,reduction='none')
                    loss_ijepa = (loss_ijepa * mask).sum() / mask.sum()
