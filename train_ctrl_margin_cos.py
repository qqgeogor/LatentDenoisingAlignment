        ...
        for i, (real_samples, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast() if args.use_amp else nullcontext():
                    ...
                    # d_loss = F.cosine_similarity(real_energy.detach(),fake_energy).abs()
                    # d_loss += F.cosine_similarity(real_energy,fake_energy.detach()).abs()
                    
                    # d_loss = 1-F.cosine_similarity(real_energy.detach(),fake_energy)
                    # d_loss += 1-F.cosine_similarity(real_energy,fake_energy.detach())
                    # d_loss = -d_loss # -1 ~ 0
                    # d_loss/=2  # 0 ~ 1

                    # margin = 0.5
                    # d_loss = F.softplus(d_loss+margin)
