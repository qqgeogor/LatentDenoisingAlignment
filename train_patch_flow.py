...
from utils_ibot import MaxSVDPatchPCANoise as PatchPCANoise
...
from vit import PatchVariationalAutoencoder,InvarianceFlow
...
def visualize_reconstruction(model,vae, images, mask_ratio=0.75, save_path='reconstructions',noise_scale=1.,pca_noiser=None):
...
def get_args_parser():
    ...
    parser.add_argument('--noise_scale', default=((1/3.)**0.5), type=float,
    ...
    # Logging and saving
    parser.add_argument('--output_dir', default='/mnt/d/repo/output/mae_cifar10_hldae_imagenet100',
...
def train_mae():
    ...
    grid = visualize_reconstruction(model,vae, imgs[:8].to(device),noise_scale=args.noise_scale,pca_noiser=pca_noiser)
        ...
        for i, (imgs, _) in enumerate(trainloader):
            ...
            if args.use_amp:
                with autocast():
                    ...
                    _, loss, loss_tcr,z = vae.forward_loss(x)
                    
                    centroid = z.reshape(b,n,-1)
                    centroid = F.normalize(centroid,dim=-1)
                    centroid = centroid.mean(dim=1)
                    
                    loss_centroid = -R_nonorm(centroid).mean()
                    loss = loss_centroid + loss
                    
                    recon_loss = (x - vae.inverse(z))**2
                    recon_loss = recon_loss.mean(-1).mean()
            ...
            if i % args.log_freq == args.log_freq - 1:
                ...
                print(f'Epoch: {epoch + 1}, Batch: {i + 1}, '
                      ...
                      f'Centroid Loss: {loss_centroid.item():.3f}, '
                      f'LR: {current_lr:.6f}')
