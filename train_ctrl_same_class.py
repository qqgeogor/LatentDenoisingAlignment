...
class SiameseDataset(Dataset):
    """
    A dataset wrapper that creates pairs of samples with the same label.
    Each item returns ((anchor, positive), label) where anchor and positive
    have the same label.
    """
    ...
    def __init__(self, dataset: Dataset):
        ...
        Args:
            base_dataset: The original dataset that returns (sample, label) pairs
        """
        self.base = dataset
        ...
        self.cache_path = os.path.join(cache_dir, f'label_to_indices_{len(dataset)}.npz')
        
        
        # Try to load from cache first
        if os.path.exists(self.cache_path):
            print(f'Loading label indices from cache: {self.cache_path}')
            cache = np.load(self.cache_path, allow_pickle=True)
            self.label_to_indices = cache['label_to_indices'].item()
        else:
            
            # Cache all labels first to avoid dataset lookups during sampling
            ...
            for label in self.unique_labels:
                ...
                self.label_to_indices[label] = np.where(self.all_labels == label)[0]
            
            # Save to cache
            print(f'Saving label indices to cache: {self.cache_path}')
            np.savez(self.cache_path, label_to_indices=self.label_to_indices)
        
        print('Number of unique labels:', len(self.label_to_indices))
    ...
    def __getitem__(self, idx: int) -> Tuple[Tuple[Any, Any], Any]:
        """
        Get a pair of samples with the same label.
        
        Args:
            idx: Index of the anchor sample
            
        Returns:
            ((anchor_sample, positive_sample), label) where both samples have the same label
        """
        # Get anchor sample and its label
        anchor, label = self.base[idx]
        
        # Get indices of all samples with the same label
        positive_indices = self.label_to_indices[label]
        
        # Select a different index for the positive sample
        pos_idx = idx
        while pos_idx == idx:
            pos_idx = random.choice(positive_indices)
            
        # Get the positive sample
        positive, _ = self.base[pos_idx]
...
def train_ebm_gan(args):
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i, ((real_samples,aug_samples), _) in tqdm(enumerate(trainloader)):
            
            # Split the batch into original and augmented samples
            # batch_size = samples.size(0) // 2
            # real_samples = samples[:batch_size]
            # aug_samples = samples[batch_size:]
