...
from utils_ibot import SVDPatchPCANoise 
import numpy as np
...
def train_ebm_gan(args):
    ...
    pca_noiser = SVDPatchPCANoise(patch_size=4,noise_scale=0.5,kernel='linear',gamma=1.0)
        ...
        for i, (real_samples, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast():
                    ...
                    with torch.no_grad():
                        noised_samples = pca_noiser(real_samples)
                    z = discriminator.net(noised_samples).squeeze()
                    fake_samples,_ = generator(z)
                    
                    d_loss = F.mse_loss(fake_samples,real_samples,reduction='none').mean(dim=(1,2,3))
                    d_loss = d_loss.mean()
...
def save_gan_samples(generator, discriminator,pca_noiser, epoch, output_dir, device, n_samples=36,real_samples=None):
    ...
    with torch.no_grad():
        ...
        # Changed 'range' to 'value_range'
        grid = make_grid(noised_samples, nrow=6, normalize=True, value_range=(-1, 1))
        plt.figure(figsize=(10, 10))
        plt.imshow(grid.cpu().permute(1, 2, 0))
        plt.axis('off')
