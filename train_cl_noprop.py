...
class DenoisingBlock(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.block = Block(embed_dim, num_heads=3, mlp_ratio=4, qkv_bias=True, qk_norm=False, proj_bias=True)   
...
class LatentDenoisingAlignment:
    def __init__(self, T=12, embed_dim=192, feature_dim=192, lr=1e-4, device='cuda'):
        self.T = T
        ...
        self.patch_embed = PatchEmbedding(img_size=28, patch_size=4, in_chans=1, embed_dim=embed_dim)
        self.mlps = nn.ModuleList([DenoisingBlock(feature_dim, embed_dim).to(device) for _ in range(T)])
        
        # Optimizers
        self.cnn_optimizer = optim.AdamW(self.patch_embed.parameters(), lr=lr)
        self.mlp_optimizers = [optim.AdamW(mlp.parameters(), lr=lr) for mlp in self.mlps]
