...
def finetune(args):
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        backbone = MaskedAutoencoderViT(img_size=args.img_size, patch_size=args.patch_size, in_chans=3,
                     ...
                     mlp_ratio=args.mlp_ratio, norm_layer=nn.LayerNorm, norm_pix_loss=False, use_checkpoint=False,
                     num_register_tokens=args.num_register_tokens).to(device)
        if args.pretrained_path:
            checkpoint = torch.load(args.pretrained_path)
            if args.model_type == 'teacher':
                backbone.load_state_dict(checkpoint['teacher_discriminator_state_dict'],strict=False)
            else:
