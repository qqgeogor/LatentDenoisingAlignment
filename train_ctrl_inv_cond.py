...
class Decoder(nn.Module):
    def __init__(self,latent_dim=128,num_classes=100):
        ...
        self.dense = nn.Linear(latent_dim, 4 * 4 * 512)
        self.reshape = Reshape((512, 4, 4))
        
        # Three ResBlocks with upsampling (up 512)
        self.resblock1 = ResBlock(512, 256, up=True)  # 4x4 -> 8x8
        self.resblock2 = ResBlock(256, 128, up=True)  # 8x8 -> 16x16
        self.resblock3 = ResBlock(128, 64, up=True)  # 16x16 -> 32x32
        self.resblock4 = ResBlock(64, 32, up=True)  # 32x32 -> 64x64
        self.resblock5 = ResBlock(32, 16, up=True)  # 64x64 -> 128x128
    ...
    def forward(self, z,labels=None):
        ...
        x = self.resblock4(x)  # -> 64x64x256
        x = self.resblock5(x)  # -> 128x128x256
        x = self.bn(x)
        x = self.relu(x)
...
class Encoder(nn.Module):
    def __init__(self, img_channels=3,latent_dim=128,num_classes=100):
        super().__init__()
        self.latent_dim = latent_dim
        self.num_classes = num_classes
        self.class_embedding = nn.Embedding(num_classes, latent_dim)
        ...
        self.resblock1 = ResBlockDown(img_channels, 32)  # 128x128 -> 64x64
        self.resblock2 = ResBlockDown(32, 64)          # 64x64 -> 32x32
        
        # Regular ResBlocks with 128 channels
        self.resblock3 = ResBlockDown(64, 128)              # 32x32 -> 16x16
        self.resblock4 = ResBlockDown(128, 256)              # 16x16 -> 8x8
        self.resblock5 = ResBlockDown(256, 512)              # 8x8 -> 4x4
        ...
        self.dense = nn.Linear(512, latent_dim)
    ...
    def net(self, x,labels=None):
        x = self.resblock1(x)
        x = self.resblock2(x)
        x = self.resblock3(x)
        # if labels is not None:
        #     class_embed = self.class_embedding(labels)
        #     x = x + class_embed.view(x.shape[0],-1,1,1)
...
def train_ebm_gan(args):
    ...
    if args.dataset == 'imagenet100':
        transform = transforms.Compose([
            ...
            transforms.RandomResizedCrop(args.img_size, scale=(0.25, 1.0)),
