        ...
        for i, (views, _) in enumerate(tqdm(trainloader)):
            ...
            for _ in range(args.n_critic):
                ...
                with autocast() if args.use_amp else nullcontext():
                    ...
                    real_energy = -F.smooth_l1_loss(z,z_anchor,reduction='none').sum(dim=-1)
                    fake_energy = -F.smooth_l1_loss(z_fake,z_anchor,reduction='none').sum(dim=-1)
            ...
            with autocast() if args.use_amp else nullcontext():
                ...
                real_cos = 1-F.cosine_similarity(z,z_anchor,dim=-1)
                fake_cos = 1-F.cosine_similarity(z_fake,z_anchor,dim=-1)
            ...
            if i % args.log_freq == 0:
                ...
                print(f'Epoch [{epoch}/{args.epochs}], Step [{i}/{len(trainloader)}], '
                      ...
                      f'Real Cos: {real_cos.mean().item():.4f}, '
                      f'Fake Cos: {fake_cos.mean().item():.4f}, '
